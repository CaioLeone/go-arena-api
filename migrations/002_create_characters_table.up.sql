CREATE TABLE IF NOT EXISTS characters (
    ...
    attribute_points INT DEFAULT 0 CHECK (attribute_points >= 0),

    -- ATRIBUTOS DO PERSONAGEM (VALORES INICIAIS PADRÃO)
...
COMMENT ON COLUMN characters.level IS 'Nível atual do personagem';
COMMENT ON COLUMN characters.experience IS 'Experiência acumulada para evolução';
COMMENT ON COLUMN characters.attribute_points IS 'Pontos livres para distribuir nos atributos';
COMMENT ON COLUMN characters.hp IS 'Health Points (vida do personagem)';
COMMENT ON COLUMN characters.attack IS 'Ataque base';
COMMENT ON COLUMN characters.defense IS 'Defesa base';
